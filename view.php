...
$id = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_NUMBER_INTO);
if (!$id) {
...
<div id="wrap">
    ...
    < id="content">
        ...
        <?php $stmt = $db->prepare('select p.id, p.member_id, p.created, m.name, m.picture from posts p, member m where p.id=? and m.id=p.member_id order by id desc');
        if (!$stmt) {
            die($db->error);
        }
