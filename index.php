...
<div id="wrap">
    ...
    <div id="content">
        ...
        <?php $stmt = $db->prepare('select p.id, p.message, p.member_id, p.created, m.name, m.picture from posts p, members m where m.id=p.member_id order by id desc');
        if (!$stmt) {
            die($db->error);
        }
        $success = $stmt->execute();
        ...
        $stmt->bind_result($id, $message, $member_id, $created, $name, $picture);
        while ($stmt->fetch()):
            ?>
        <div class="msg">
            <?php if ($picture): ?>
